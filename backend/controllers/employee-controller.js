...
exports.createEmployee = async (req, res, next) => {
  try {
    ...
    if (e.code === 11000){ 
      e.status = 409; e.message = 'Employee email already exists'; 
    }
    next(e);
  }
};
...
exports.deleteEmployee = async (req, res, next) => {
  try {
    ...
    if (!employee_id){
      return res.status(400).json({ status: false, message: 'Error: employee_id is required' });
    } 
    
    const deleted = await Employee.findByIdAndDelete(employee_id);

    if (!deleted){
      return res.status(404).json({ status: false, message: 'Error: Employee not found' });
    }
